...
byte walls[16][16];       
byte dist[16][16];
byte visited[16][16]; // Tracks cells actually driven on
...
// false = Going to Center, true = Returning to Start
...
// ================================================================
//               SEARCH LOGIC (TWO-WAY: Start->Center->Start)
// ================================================================
void runSearchLogic() {
  // 1. Mark Visited
  visited[curX][curY] = 1;
  ...
  if (dist[curX][curY] == 0) {
    ...
    if (!searchingReturn) {
        ...
        // The robot is now ready to navigate back. 
        // We return here to let the loop run again and calculate the first move back.
        return; 
    }
    
    // --- PHASE 2 DONE: Returned to Start ---
    else {
        // We are back at 0,0. The map is verified.
        saveMapToEEPROM();
        
        // Victory Song (Longer to indicate save complete)
  ...
  for(int i=0; i<4; i++) {
    ...
    if (!isWall(curX, curY, dir)) {
      ...
      if (nx >= 0 && nx < MAZE_SIZE && ny >= 0 && ny < MAZE_SIZE) {
         ...
         // This prevents the "Phantom Path" crash.
         if (visited[nx][ny] == 1) {
             if (dist[nx][ny] < minDist) { minDist = dist[nx][ny]; bestDir = dir; }
         }
      }
